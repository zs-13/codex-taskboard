...
test("the issue workspace projects the seven statuses into adaptive main and secondary groups", () => {
  assert.deepEqual(statusList("MAIN_STATUSES"), ["backlog", "todo", "in_progress", "blocked", "in_review", "done"]);
  assert.deepEqual(statusList("SECONDARY_STATUSES"), ["canceled"]);
  assert.match(statusSource, /satisfies readonly TaskStatus\[\]/);
  assert.match(appSource, /const mainStatuses = MAIN_STATUSES;/);
  ...
  assert.match(boardColumnSource, /backlog: \{ label: "需求池", tone: "backlog" \}/);
  assert.match(boardColumnSource, /todo: \{ label: "待认领", tone: "todo" \}/);
  assert.match(boardColumnSource, /in_progress: \{ label: "执行中", tone: "progress" \}/);
  assert.match(boardColumnSource, /blocked: \{ label: "阻塞", tone: "blocked" \}/);
  assert.match(boardColumnSource, /in_review: \{ label: "待验收", tone: "review" \}/);
