...
> **New to Taskboard?** Start here: [Beginner's guide (简体中文)](docs/beginners-guide.md) — step-by-step install, start, and first tasks.
...
When auto-execution is on, claim/assign assigns the executor and starts the headless turn while the task is still `todo`; the headless turn then claims the task itself with its own conversation binding, so the lock belongs to the executor and the task only moves to `in_progress` when real execution starts (pre-claiming from the runner caused fake execution — status showed running/completed with no work). The task transitions through `executionState` (`claimed` → `running` → `completed`/`failed`/`interrupted`; back to `idle` if the turn ends without claiming). The task card and detail panel show that state, and the headless agent streams progress comments into the task without you opening a conversation. Stuck tasks whose lock has expired and have no active turn can be recovered via `POST /api/tasks/:id/recover` (the agent runner does this automatically).
