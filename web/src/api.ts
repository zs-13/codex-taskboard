...
export async function listAgents(signal?: AbortSignal): Promise<AgentProfile[]> {
  const data = await request<{ agents: AgentProfile[] }>("/api/agents", { signal });
  return data.agents;
}
...
export async function listSquads(signal?: AbortSignal): Promise<Squad[]> {
  const data = await request<{ squads: Squad[] }>("/api/squads", { signal });
  return data.squads;
}
...
export async function listGlobalActivities(signal?: AbortSignal): Promise<GlobalActivity[]> {
  const data = await request<{ activities: GlobalActivity[] }>("/api/activity-log?limit=100", { signal });
  return data.activities;
}

export async function assignTask(
  task: Task,
  input: { agentId: string | null; squadId: string | null },
): Promise<Task> {
  const data = await request<{ task: Task }>(`/api/tasks/${encodeURIComponent(task.id)}/assign`, {
    method: "POST",
    body: JSON.stringify({ version: task.version, ...input }),
  });
  return data.task;
}

export async function setTaskBlocked(task: Task, blocked: boolean, reason: string): Promise<Task> {
  const data = await request<{ task: Task }>(`/api/tasks/${encodeURIComponent(task.id)}/block`, {
    method: "POST",
    body: JSON.stringify({ version: task.version, blocked, reason }),
  });
  return data.task;
}
...
export async function controlTask(
  task: Task,
  controlStatus: "active" | "paused" | "terminated",
): Promise<Task> {
  const data = await request<{ task: Task }>(`/api/tasks/${encodeURIComponent(task.id)}/control`, {
    method: "POST",
    body: JSON.stringify({ version: task.version, controlStatus }),
  });
  return data.task;
}

export async function claimTask(task: Task, agentId: string | null): Promise<Task> {
  const data = await request<{ task: Task }>(`/api/tasks/${encodeURIComponent(task.id)}/claim`, {
    method: "POST",
    body: JSON.stringify({ agentId, ttlSeconds: 900 }),
  });
  return data.task;
}

export async function addTaskDependency(task: Task, dependsOnTaskId: string): Promise<Task> {
  const data = await request<{ task: Task }>(`/api/tasks/${encodeURIComponent(task.id)}/dependencies`, {
    method: "POST",
...
export async function runAutonomousSquadStep(task: Task): Promise<Task> {
  const data = await request<{ task: Task }>(`/api/tasks/${encodeURIComponent(task.id)}/autonomous-step`, {
    method: "POST",
    body: JSON.stringify({}),
  });
  return data.task;
