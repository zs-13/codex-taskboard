...
/**
 * Tasks auto-generated by the squad autonomous runner (parent titled
 * `Squad autonomous <id>` plus its step subtasks) are execution noise, not
 * user work. The board's "只看任务" mode hides them by title rule.
 */
export function isNoiseTask(task: Task): boolean {
  return task.title.startsWith("Squad autonomous ");
}
...
export function matchesTaskFilters(
  ...
  filters: TaskFilters,
  omit?: TaskFilterKey,
): boolean {  if (omit !== "statuses" && filters.statuses.length && !filters.statuses.includes(task.status)) {
    return false;
  }
