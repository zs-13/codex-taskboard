...
function parseStringList(value, name, { maxItems = 20, maxLength = 64 } = {}) {
  if (value === undefined) return [];
  if (!Array.isArray(value) || value.length > maxItems) {
    throw new ApiError(400, "INVALID_FIELD", `'${name}' must be an array with at most ${maxItems} entries`);
  }
  const result = value.map((entry) => stringField(entry, name, { required: true, maxLength }));
  if (new Set(result).size !== result.length) {
    throw new ApiError(400, "INVALID_FIELD", `'${name}' entries must be unique`);
  }
  return result;
}
...
function parseSquadCreate(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["id", "name", "leaderAgentId", "memberAgentIds", "skillTags"]));
  return {
    id: stringField(body.id, "id", { maxLength: 96 }),
    name: stringField(body.name, "name", { required: true, maxLength: 120 }),
    leaderAgentId: stringField(body.leaderAgentId, "leaderAgentId", { required: true, maxLength: 96 }),
    memberAgentIds: parseStringList(body.memberAgentIds, "memberAgentIds", { maxItems: 50, maxLength: 96 }),
    skillTags: parseStringList(body.skillTags, "skillTags"),
  };
}
...
function parseSkillTemplateCreate(body) {  assertPlainObject(body);
  ...
  return {
    id: stringField(body.id, "id", { maxLength: 96 }),
    name: stringField(body.name, "name", { required: true, maxLength: 120 }),
    description: stringField(body.description ?? "", "description", { maxLength: 2000 }),
...
function parseTaskBlock(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["version", "blocked", "reason"]));
  if (typeof body.blocked !== "boolean") {
    throw new ApiError(400, "INVALID_FIELD", "'blocked' must be true or false");
  }
  return {
    version: parseVersion(body.version),
    blocked: body.blocked,
    reason: stringField(body.reason ?? "", "reason", { maxLength: 2000 }),
  };
}

function parseTaskControl(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["version", "controlStatus"]));
  if (!["active", "paused", "terminated"].includes(body.controlStatus)) {
    throw new ApiError(400, "INVALID_FIELD", "'controlStatus' must be active, paused, or terminated");
  }
  return { version: parseVersion(body.version), controlStatus: body.controlStatus };
}

function parseTaskLock(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["owner", "ownerSessionId", "ttlSeconds"]));
  const ttlSeconds = body.ttlSeconds === undefined ? 900 : body.ttlSeconds;
  if (!Number.isSafeInteger(ttlSeconds) || ttlSeconds < 30 || ttlSeconds > 86_400) {
    throw new ApiError(400, "INVALID_FIELD", "'ttlSeconds' must be an integer from 30 to 86400");
  }
  return {
    owner: stringField(body.owner, "owner", { required: true, maxLength: 120 }),
    ownerSessionId: stringField(body.ownerSessionId ?? null, "ownerSessionId", { nullable: true, maxLength: 256 }),
    expiresAt: new Date(Date.now() + ttlSeconds * 1000).toISOString(),
  };
}

function parseTaskClaim(body) {
  assertPlainObject(body);
  assertAllowedKeys(body, new Set(["agentId", "ownerSessionId", "ttlSeconds"]));
  const ttlSeconds = body.ttlSeconds === undefined ? 900 : body.ttlSeconds;
  if (!Number.isSafeInteger(ttlSeconds) || ttlSeconds < 30 || ttlSeconds > 86_400) {
    throw new ApiError(400, "INVALID_FIELD", "'ttlSeconds' must be an integer from 30 to 86400");
  }
...
function parseTaskDependency(body) {
  ...
  return {
    dependsOnTaskId: stringField(body.dependsOnTaskId, "dependsOnTaskId", { required: true, maxLength: 128 }),
  };
}
...
function extractMentions(value) {
  const mentions = [];
  for (const match of String(value).matchAll(/@([\p{L}\p{N}_.-]{1,64})/gu)) {
...
class EventHub {
  constructor() {
    ...
    this.keepAlive = setInterval(() => {
      ...
      for (const socket of this.webSockets) this.#writeFrame(socket, JSON.stringify({
        type: "ping",
  ...
  connectWebSocket(request, socket) {
    ...
    socket.on("data", (chunk) => this.#handleFrame(socket, chunk));
    socket.once("close", () => this.webSockets.delete(socket));
    socket.once("error", () => this.webSockets.delete(socket));
    this.#writeFrame(socket, JSON.stringify({
      type: "sync.required",
      message: "Connected. Refresh local state for consistency.",
      at: new Date().toISOString(),
    }));
  }
  ...
  emit(type, value) {
    ...
    for (const socket of this.webSockets) this.#writeFrame(socket, frame);
  }
  ...
  close() {
    ...
    this.clients.clear();
    for (const socket of this.webSockets) socket.end();
    this.webSockets.clear();
  }

  #handleFrame(socket, chunk) {
    ...
    let mask;
    if (masked) {
      if (chunk.length < offset + 4) return;
      mask = chunk.subarray(offset, offset + 4);
      offset += 4;
    }
    if (chunk.length < offset + length || length > 64 * 1024) return;
    ...
    if (message?.type === "ping" || message?.type === "sync") {
      this.#writeFrame(socket, JSON.stringify({
        type: message.type === "sync" ? "sync.required" : "pong",
        at: new Date().toISOString(),
      }));
    }
  }
  ...
  #writeFrame(socket, text) {
    ...
    if (payload.length > 65535) return;
...
export function createTaskboardServer(options = {}) {
  ...
  // Return a stuck/orphaned task (pre-claimed, marked completed/failed, but
  // never actually executed) to the claimable pool so the runner can re-trigger
  // it under the executor-claims-itself mechanism. Only a terminal execution
  // state with no conversation binding is eligible: claimed/running means a turn
  // is live, a thread binding means a turn legitimately owns the task, and a
  // valid lock means someone still holds it.
    ...
    try {
      ...
      if (resolved.instanceToken && incomingUrl.pathname !== "/health") {
        if (incomingUrl.pathname === "/") {
          response.writeHead(302, { location: `${routePrefix}/${incomingUrl.search}` });
          response.end();
          return;
        }
      ...
      const earlyCloudConfig = pathname.startsWith("/api/")
        ? await cloudConfig.read()
        : null;
      if (earlyCloudConfig?.remoteUrl) assertLoopbackRequest(request);
      ...
      let currentCloudConfig = earlyCloudConfig;
      ...
      if (pathname === "/api/tasks") {
        ...
        if (request.method === "POST") {
          ...
          const replay = database.getIdempotencyResponse(key, request.method, pathname, requestHash);
          if (replay) return sendJson(response, 200, replay);
          const actor = actorFromRequest(request);
          const { assigneeTarget, ...parsedInput } = parseTaskCreate(body);
          ...
          const payload = { task };
          database.storeIdempotencyResponse(key, request.method, pathname, payload, requestHash);
          database.recordActivity({
            projectId: task.projectId,
            taskId: task.id,
            actor,
            eventType: "task.created",
            message: `Task ${task.identifier} created`,
            payload: { idempotencyKey: key ?? null },
          });
      ...
      if (pathname === "/api/agents") {
        if (request.method === "GET") return sendJson(response, 200, { agents: database.listAgents() });
        if (request.method === "POST") {
          const key = requestHeader(request, "idempotency-key");
          const body = await readJson(request);
          const requestHash = idempotencyHash(body);
          const replay = database.getIdempotencyResponse(key, request.method, pathname, requestHash);
          if (replay) return sendJson(response, 200, replay);
          const agent = database.upsertAgent(parseAgentUpsert(body), actorFromRequest(request));
          const payload = { agent };
          database.storeIdempotencyResponse(key, request.method, pathname, payload, requestHash);
          events.emit("agent.upserted", { agent });
          return sendJson(response, 201, payload);
        }
        return methodNotAllowed(response, ["GET", "POST"]);
      }
      ...
      if (pathname === "/api/squads") {
        ...
        if (request.method === "POST") {
          ...
          const replay = database.getIdempotencyResponse(key, request.method, pathname, requestHash);
          if (replay) return sendJson(response, 200, replay);
          const squad = database.createSquad(parseSquadCreate(body), actorFromRequest(request));
          const payload = { squad };
          database.storeIdempotencyResponse(key, request.method, pathname, payload, requestHash);
          events.emit("squad.created", { squad });
          return sendJson(response, 201, payload);
        }
        return methodNotAllowed(response, ["GET", "POST"]);
      }
      ...
      if (pathname === "/api/skills") {
        if (request.method === "GET") return sendJson(response, 200, { skills: database.listSkillTemplates() });
        if (request.method === "POST") {
          const key = requestHeader(request, "idempotency-key");
          const body = await readJson(request);
          const requestHash = idempotencyHash(body);
          const replay = database.getIdempotencyResponse(key, request.method, pathname, requestHash);
          if (replay) return sendJson(response, 200, replay);
          const skill = database.createSkillTemplate(parseSkillTemplateCreate(body), actorFromRequest(request));
          const payload = { skill };
          database.storeIdempotencyResponse(key, request.method, pathname, payload, requestHash);
          events.emit("skill.created", { skill });
          return sendJson(response, 201, payload);
        }
        return methodNotAllowed(response, ["GET", "POST"]);
      }
      ...
      if (extendedTaskRoute) {
        ...
        const action = extendedTaskRoute[2];
        if (request.method !== "POST") return methodNotAllowed(response, ["POST"]);
        const actor = actorFromRequest(request);
        let task;
        if (action === "assign") {
        ...
        if (action === "block") {
          const input = parseTaskBlock(await readJson(request));
          task = database.setTaskBlocked(taskId, input.version, input, actor);
          events.emit(input.blocked ? "task.blocked" : "task.unblocked", { task });
          return sendJson(response, 200, { task });
        }
        if (action === "control") {
          const input = parseTaskControl(await readJson(request));
          task = database.requestTaskControl(taskId, input.version, input.controlStatus, actor);
          events.emit("task.control.updated", { task });
          return sendJson(response, 200, { task });
        }
        if (action === "lock") {
          task = database.acquireTaskLock(taskId, parseTaskLock(await readJson(request)), actor);
          events.emit("task.locked", { task });
          return sendJson(response, 200, { task });
        }
        if (action === "claim") {
        ...
        if (action === "dependencies") {
          ...
          task = database.addTaskDependency(taskId, input.dependsOnTaskId, actor);
          events.emit("task.dependency.updated", { task });
          return sendJson(response, 200, { task });
        }
        task = database.runAutonomousSquadStep(taskId, actor);
        events.emit("squad.autonomous.step", { task });
        return sendJson(response, 200, { task });
      }
      ...
      if (taskCommentsRoute) {
        ...
        if (request.method === "POST") {
          ...
          database.recordActivity({
            ...
            message: `Comment added to ${task.identifier}`,
            payload: { commentId: comment.id, mentions: extractMentions(input.body) },
          });
      ...
      if (taskRoute) {
        ...
        if (!action && request.method === "PATCH") {
          ...
          if (task.status === "done") {
            for (const startedTask of database.startUnblockedDependents(task.id, actor)) {
              events.emit("task.dependency.released", { task: startedTask });
            }
          }
          return sendJson(response, 200, { task });
        }
        ...
        if (action === "move" && request.method === "POST") {
          ...
          if (task.status === "done") {
            for (const startedTask of database.startUnblockedDependents(task.id, actorFromRequest(request))) {
              events.emit("task.dependency.released", { task: startedTask });
            }
          }
          return sendJson(response, 200, { task });
  ...
  server.on("upgrade", (request, socket) => {
    try {
      const incomingUrl = new URL(request.url, "http://127.0.0.1");
      const upgradePathname = resolved.instanceToken
        && incomingUrl.pathname.startsWith(`${routePrefix}/`)
        ? incomingUrl.pathname.slice(routePrefix.length)
        : incomingUrl.pathname;
      if (upgradePathname !== "/api/ws") {
        socket.destroy();
        return;
      }
      request.url = `${upgradePathname}${incomingUrl.search}`;
      assertTrustedNetworkRequest(request, true);
      events.connectWebSocket(request, socket);
    } catch {
      socket.destroy();
    }
  });
