...
function taskFromRow(row) {
  ...
  return {
    ...
    controlStatus: row.control_status ?? "active",
    controlRequestedAt: row.control_requested_at ?? null,
...
export class TaskboardDatabase {
  ...
  #migrate() {
    ...
    const extendedTaskColumns = this.database.prepare("PRAGMA table_info(tasks)").all();
    for (const [column, definition] of [
      ...
      if (!extendedTaskColumns.some((candidate) => candidate.name === column)) {
        this.database.exec(`ALTER TABLE tasks ADD COLUMN ${column} ${definition}`);
      }
    }
    ...
    this.database.exec(`
      ...
      CREATE TABLE IF NOT EXISTS task_dependencies (
        ...
        condition TEXT NOT NULL DEFAULT 'done',
        created_at TEXT NOT NULL,
  ...
  upsertAgent(input, actor) {
    ...
    this.recordActivity({
      ...
      message: `Agent ${agent.name} saved`,
      payload: { agentId: id },
    });
    return agent;
  }
  ...
  listSquads() {
    const squads = this.database.prepare(`
      SELECT squads.*, agents.name AS leader_name
      FROM squads
      LEFT JOIN agents ON agents.id = squads.leader_agent_id
      ORDER BY squads.name, squads.id
    `).all().map((row) => ({
      id: row.id,
      name: row.name,
    ...
    const members = this.database.prepare(`
      SELECT squad_members.*, agents.name AS agent_name
      FROM squad_members
      LEFT JOIN agents ON agents.id = squad_members.agent_id
      ORDER BY squad_members.created_at, squad_members.agent_id
    `).all();
    const byId = new Map(squads.map((squad) => [squad.id, squad]));
    for (const member of members) {
      byId.get(member.squad_id)?.members.push({
  ...
  createSquad(input, actor) {
    ...
    try {
      this.database.prepare(`
        INSERT INTO squads (id, name, leader_agent_id, skill_tags, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?)
      ...
      for (const agentId of memberIds) {
        ...
        this.database.prepare(`
          ...
          VALUES (?, ?, ?, ?)
        `).run(id, agentId, agentId === input.leaderAgentId ? "leader" : "member", timestamp);
      }
      this.database.exec("COMMIT");
    } catch (error) {
      this.database.exec("ROLLBACK");
    ...
    this.recordActivity({
      ...
      message: `Squad ${squad.name} created`,
      payload: { squadId: id },
    });
    return squad;
  }
  ...
  createSkillTemplate(input, actor) {
    const timestamp = now();
    const id = input.id || randomUUID();
    this.database.prepare(`
      INSERT INTO skill_templates (id, name, description, body, skill_tags, created_at, updated_at)
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `).run(id, input.name, input.description, input.body, JSON.stringify(input.skillTags), timestamp, timestamp);
    const template = this.listSkillTemplates().find((candidate) => candidate.id === id);
    this.recordActivity({
      projectId: null,
      taskId: null,
      actor,
      eventType: "skill_template.created",
      message: `Skill template ${template.name} saved`,
      payload: { skillTemplateId: id },
    });
    return template;
  }
  ...
  assignTask(id, version, input, actor) {
    ...
    this.recordActivity({
      ...
      message: `Task ${task.identifier} assigned`,
      payload: { agentId: input.agentId, squadId: input.squadId },
    });
    return task;
  }
  ...
  setTaskBlocked(id, version, input, actor) {
    ...
    this.#requireVersion(current, version);
    const timestamp = now();
    const blocked = input.blocked ? 1 : 0;
    this.database.exec("BEGIN IMMEDIATE");
    try {
      const result = this.database.prepare(`
        UPDATE tasks
        SET blocked = ?, blocked_reason = ?, blocked_by = ?, status = ?, version = version + 1, updated_at = ?
        WHERE id = ? AND version = ?
      `).run(
        blocked,
        input.blocked ? input.reason : null,
        input.blocked ? actor.id : null,
        input.blocked ? "blocked" : current.status === "blocked" ? "todo" : current.status,
        timestamp,
        current.id,
        version,
      );
      ...
      this.#recordTaskActivity(current.id, actor, taskFieldChanges(current, {
        ...
        blockedReason: input.blocked ? input.reason : null,
        blockedBy: input.blocked ? actor.id : null,
        status: input.blocked ? "blocked" : current.status === "blocked" ? "todo" : current.status,
      }), timestamp);
      this.database.exec("COMMIT");
    } catch (error) {
      this.database.exec("ROLLBACK");
    ...
    this.recordActivity({
      ...
      taskId: task.id,
      actor,
      eventType: input.blocked ? "task.blocked" : "task.unblocked",
      message: input.blocked ? `Task ${task.identifier} blocked` : `Task ${task.identifier} unblocked`,
      payload: { reason: input.reason ?? null },
    });
    return task;
  }

  requestTaskControl(id, version, controlStatus, actor) {
    const current = this.#requireTask(id);
    this.#requireVersion(current, version);
    const timestamp = now();
    const result = this.database.prepare(`
      UPDATE tasks
      SET control_status = ?, control_requested_at = ?, version = version + 1, updated_at = ?
      WHERE id = ? AND version = ?
    `).run(controlStatus, timestamp, timestamp, current.id, version);
    if (result.changes !== 1) this.#throwMissingOrConflict(id, version);
    const task = this.getTask(current.id);
    this.recordActivity({
      projectId: task.projectId,
      taskId: task.id,
      actor,
      eventType: `task.${controlStatus}`,
      message: `Task ${task.identifier} control set to ${controlStatus}`,
      payload: { controlStatus },
    });
    return task;
  }

  acquireTaskLock(taskId, input, actor) {
    const task = this.#requireTask(taskId);
    const timestamp = now();
    const existing = this.database.prepare(`
      SELECT * FROM task_locks WHERE task_id = ?
    `).get(task.id);
    if (existing && existing.expires_at > timestamp && existing.owner !== input.owner) {
    ...
    try {
      this.database.prepare(`
        INSERT INTO task_locks (task_id, owner, owner_session_id, expires_at, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?)
        ON CONFLICT(task_id) DO UPDATE SET
          ...
          updated_at = excluded.updated_at
      `).run(task.id, input.owner, input.ownerSessionId, input.expiresAt, timestamp, timestamp);
      this.database.prepare(`
        UPDATE tasks
        SET lock_owner = ?, lock_expires_at = ?, updated_at = ?
        WHERE id = ?
      `).run(input.owner, input.expiresAt, timestamp, task.id);
      this.database.exec("COMMIT");
    } catch (error) {
      this.database.exec("ROLLBACK");
      throw error;
    ...
    this.recordActivity({
      ...
      taskId: task.id,
      actor,
      eventType: "task.locked",
      message: `Task ${task.identifier} locked by ${input.owner}`,
      payload: input,
    });
    return this.getTask(task.id);
  }

  claimTaskForAgent(taskId, input, actor) {
    const task = this.#requireTask(taskId);
    if (task.blocked || task.status === "blocked") {
      throw new ApiError(409, "TASK_BLOCKED", "Blocked tasks cannot be claimed until the blocker is cleared");
    }
    ...
    try {
      ...
      this.database.prepare(`
        ...
        ON CONFLICT(task_id) DO UPDATE SET
          ...
          expires_at = excluded.expires_at,
          updated_at = excluded.updated_at
      `).run(task.id, selectedAgent.id, input.ownerSessionId, input.expiresAt, timestamp, timestamp);
      this.database.prepare(`
        ...
        ) VALUES (?, ?, ?, NULL, NULL, NULL, NULL, NULL, 'agent', ?, ?, ?, 1, ?, ?)
      `).run(
        randomUUID(),
        task.id,
        `@${selectedAgent.name} claimed this task, acquired the execution lock, and started autonomous work.`,
        selectedAgent.id,
        selectedAgent.name,
      ...
      this.#recordTaskActivity(task.id, { type: "agent", id: selectedAgent.id, name: selectedAgent.name, avatarUrl: selectedAgent.avatarUrl }, taskFieldChanges(task, {
        assignedAgentId: selectedAgent.id,
        status: task.status === "backlog" || task.status === "todo" ? "in_progress" : task.status,
        lockOwner: selectedAgent.id,
    ...
    this.recordActivity({
      ...
      taskId: updated.id,
      actor: { type: "agent", id: selectedAgent.id, name: selectedAgent.name, avatarUrl: selectedAgent.avatarUrl },
      eventType: "agent.task_claimed",
      message: `Agent ${selectedAgent.name} claimed ${updated.identifier}`,
  ...
  addTaskDependency(taskId, dependsOnTaskId, actor) {
    ...
    this.#validateRelationTasks(task, dependency);
    const timestamp = now();
    this.database.exec("BEGIN IMMEDIATE");
    try {
      this.database.prepare(`
      ...
      `).run(task.id, dependency.id, timestamp);
      this.database.prepare(`
      ...
      `).run(dependency.id, task.id, timestamp);
      this.database.exec("COMMIT");
    } catch (error) {
      this.database.exec("ROLLBACK");
      throw error;
    ...
    this.recordActivity({
      ...
      message: `${task.identifier} waits for ${dependency.identifier}`,
      payload: { dependsOnTaskId: dependency.id },
    });
    return this.getTask(task.id);
  }

  startUnblockedDependents(completedTaskId, actor) {
    const timestamp = now();
    const rows = this.database.prepare(`
      ...
      WHERE task_dependencies.depends_on_task_id = ?
        ...
        AND NOT EXISTS (
          SELECT 1
          FROM task_dependencies AS pending
          JOIN tasks AS dependency ON dependency.id = pending.depends_on_task_id
          WHERE pending.task_id = task_dependencies.task_id
    ...
    for (const row of rows) {
      ...
      const result = this.database.prepare(`
        UPDATE tasks
        SET status = 'todo', blocked = 0, blocked_reason = NULL, blocked_by = NULL,
          version = version + 1, updated_at = ?
        WHERE id = ? AND version = ?
      `).run(timestamp, row.id, row.version);
      if (result.changes === 1) {
        this.#recordTaskActivity(task.id, actor, taskFieldChanges(task, {
          status: "todo",
          blocked: false,
          blockedReason: null,
          blockedBy: null,
        }), timestamp);
        started.push(this.getTask(row.id));
      }
    }
    return started;
  }

  runAutonomousSquadStep(taskId, actor) {
    const task = this.#requireTask(taskId);
    if (!task.squadId) {
      throw new ApiError(409, "SQUAD_REQUIRED", "Assign the task to a squad before autonomous execution");
    }
    const squad = this.listSquads().find((candidate) => candidate.id === task.squadId);
    if (!squad) throw new ApiError(404, "SQUAD_NOT_FOUND", `Squad '${task.squadId}' does not exist`);
    const timestamp = now();
    const members = squad.members.length > 0 ? squad.members : [{ agentId: squad.leaderAgentId, agentName: squad.leaderName, role: "leader" }];
    try {
      const existingChildren = this.database.prepare(`
      ...
      if (existingChildren === 0) {
        const steps = [
          ...
          "Self-check and prepare review notes",
        ];
        let index = 0;
        for (const step of steps) {
          ...
          const child = this.createTask({
            ...
            description: `Autonomous squad subtask generated from ${task.identifier}.`,
            status: "todo",
            priority: task.priority,
            labels: [...new Set([...task.labels, "squad"])],
            sortOrder: undefined,
            threadId: undefined,
            threadBinding: undefined,
            actor,
            assignee: { type: "agent", id: agent.agentId, name: agent.agentName ?? agent.agentId, avatarUrl: null },
            workflowId: null,
          ...
          this.database.prepare(`
            INSERT INTO task_relations (relation_type, source_task_id, target_task_id, created_at)
            VALUES ('parent', ?, ?, ?)
          `).run(task.id, child.id, timestamp);
          this.database.prepare(`
            UPDATE tasks SET assigned_agent_id = ?, squad_id = ?, updated_at = ? WHERE id = ?
          `).run(agent.agentId, squad.id, timestamp, child.id);
          index += 1;
        }
      }
      this.database.prepare(`
        UPDATE tasks
        SET status = CASE WHEN status IN ('backlog', 'todo') THEN 'in_progress' ELSE status END,
          last_autonomous_at = ?, version = version + 1, updated_at = ?
        WHERE id = ?
      ...
      this.database.prepare(`
        INSERT INTO comments (
          ...
          author_type, author_id, author_name, author_avatar_url,
          version, created_at, updated_at
        ) VALUES (?, ?, ?, NULL, NULL, NULL, NULL, NULL, 'agent', ?, ?, NULL, 1, ?, ?)
      `).run(
        randomUUID(),
        task.id,
        `@${squad.name} autonomous step started. The leader split the task, assigned members, and moved execution forward.`,
        squad.leaderAgentId,
        squad.leaderName ?? squad.leaderAgentId,
        timestamp,
        timestamp,
      );
    } catch (error) {
    ...
    this.recordActivity({
      ...
      taskId: updated.id,
      actor,
      eventType: "squad.autonomous_step",
      message: `Squad ${squad.name} advanced ${updated.identifier}`,
      payload: { squadId: squad.id },
    });
    return updated;
  }
  ...
  recordActivity({ projectId, taskId, actor, eventType, message, payload = {} }) {
    const resolvedActor = actor ?? { type: "system", id: "system", name: "System", avatarUrl: null };
    this.database.prepare(`
      INSERT INTO activity_log (
        id, project_id, task_id, actor_type, actor_id, actor_name, event_type, message, payload, created_at
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(
      randomUUID(),
      projectId,
  ...
  getIdempotencyResponse(key, method, pathname, requestHash) {
    if (!key) return null;
    const row = this.database.prepare(`
      SELECT request_hash, response_json FROM idempotency_keys
      WHERE key = ? AND method = ? AND pathname = ?
    `).get(key, method, pathname);
    if (row && row.request_hash && requestHash && row.request_hash !== requestHash) {
      throw new ApiError(409, "IDEMPOTENCY_CONFLICT", "Idempotency key was already used with a different request body");
    }
    return row ? JSON.parse(row.response_json) : null;
  }

  storeIdempotencyResponse(key, method, pathname, response, requestHash = null) {
    if (!key) return;
    this.database.prepare(`
      INSERT INTO idempotency_keys (key, method, pathname, request_hash, response_json, created_at)
      VALUES (?, ?, ?, ?, ?, ?)
      ON CONFLICT(key) DO NOTHING
    `).run(key, method, pathname, requestHash, JSON.stringify(response), now());
  }
